...
nslvar_yz=10
nslvar_xz=sum(param_dict['xz_var'])
print('nslvar=',nslvar)
...
xz_lev=param_dict['xz_lev']
if len(xz_lev) == 1 and xz_lev[0]==0:
    xz_slice=0
else:
    xz_slice=1
format_xz_lev=[]
for i in xz_lev:
    format_xz_lev.append( "{:04d}".format(i))
...
with open('compression_parameters.json', 'r') as json_file:
    # Load the JSON content into a Python dictionary
    comp_level = json.load(json_file)
...
for k,v in comp_level.items():
    if comp_level[k]['type']=='zarr.Sperr':
        comp_level[k]['func'] = dict(compressor=zarr.Sperr(mode=comp_level[k]['mode'],level=comp_level[k]['level'],pre=comp_level[k]['pre']))
    elif comp_level[k]['type']=='zarr.GZip':
        comp_level[k]['func']=dict(compressor=zarr.GZip(level=comp_level[k]['level']))
...
for i in range(nsteps):
    ...
    if multifile:
        ...
        shape_xz = (nslvar_xz, gsize[0], gsize[2])
        chunks_xz = (nslvar_xz, lsize[0], lsize[2])
    ...
    if singlefile or multifile:
        ...
        if xz_slice:
            store_xz = zarr.DirectoryStore(f'2D/xz_slice.{ni}.lossless.single.122.zarr')
        ...
        if need_zip:
            ...
            if xz_slice:
                store_xz = zarr.ZipStore(f'2D/xz_slice.{ni}.lossless.single.122.zip', mode='w')
        ...
        if rank == 0:
            ...
            if xz_slice:
                prim_xz = zarr.group(store=store_xz, overwrite=True)
            
            prim_tau = zarr.group(store=store_tau, overwrite=True)
            if dem:
                prim_corona = zarr.group(store=store_corona, overwrite=True)
            prim_0 = prim.create_dataset('result_prim_0', shape=shape1, chunks=chunks1,dtype=dtype,**zarr_compression_kw,order='F')
            #prim_0 = prim.create_dataset('prim_0', shape=shape1, chunks=chunks1,dtype=dtype)
            prim_1 = prim.create_dataset('result_prim_1', shape=shape1, chunks=chunks1,dtype=dtype,**zarr_compression_kw,order='F')
            prim_2 = prim.create_dataset('result_prim_2', shape=shape1, chunks=chunks1,dtype=dtype,**zarr_compression_kw,order='F')
            prim_3 = prim.create_dataset('result_prim_3', shape=shape1, chunks=chunks1,dtype=dtype,**zarr_compression_kw,order='F')
            prim_4 = prim.create_dataset('result_prim_4', shape=shape1, chunks=chunks1,dtype=dtype,**zarr_compression_kw,order='F')
            prim_5 = prim.create_dataset('result_prim_5', shape=shape1, chunks=chunks1,dtype=dtype,**zarr_compression_kw,order='F')
            prim_6 = prim.create_dataset('result_prim_6', shape=shape1, chunks=chunks1,dtype=dtype,**zarr_compression_kw,order='F')
            prim_7 = prim.create_dataset('result_prim_7', shape=shape1, chunks=chunks1,dtype=dtype,**zarr_compression_kw,order='F')
            prim_8 = prim.create_dataset('result_prim_8', shape=shape1, chunks=chunks1,dtype=dtype,**zarr_compression_kw,order='F')
            diag=[]
            ...
            if xz_slice:
                for vi in format_xz_lev:
                    xz_slice = prim_xz.create_dataset(f'xz_slice_{vi}', shape=shape_xz, chunks=chunks_xz, dtype=dtype, **zarr_compression_kw, order='F')
            ...
            if xz_slice:
                for vi in format_xz_lev:
                    prim_xz[f'xz_slice_{vi}'].attrs['_ARRAY_DIMENSIONS'] = dim3D_2
        ...
        if xz_slice:
            prim2_xz = zarr.open_group(store=store_xz,mode='a')
    ...
    count_xz = [nslvar_xz,count[0], count[2]]
    start_xz = [0,start[0], start[2]]
    end_xz = [count_xz[0],end[0], end[2]]
    ...
    if bpFileReader.BeginStep()==adios2.StepStatus.OK:
        ...
        var_get = bpIO.InquireVariable(f"result_prim_0.{begin_iter}")
        var_get1 = bpIO.InquireVariable(f"result_prim_1.{begin_iter}")
        var_get2 = bpIO.InquireVariable(f"result_prim_2.{begin_iter}")
        var_get3 = bpIO.InquireVariable(f"result_prim_3.{begin_iter}")
        var_get4 = bpIO.InquireVariable(f"result_prim_4.{begin_iter}")
        var_get5 = bpIO.InquireVariable(f"result_prim_5.{begin_iter}")
        var_get6 = bpIO.InquireVariable(f"result_prim_6.{begin_iter}")
        var_get7 = bpIO.InquireVariable(f"result_prim_7.{begin_iter}")
        var_get8 = bpIO.InquireVariable(f"result_prim_8.{begin_iter}")
    ...
    print('inVar value=',inVar[0,0,0:10])
    print('inVar1 value=',inVar1[0,0,0:10])
    ...
    if yz_slice:
        ...
        for c,vi in var_get_yz.items():
            ...
            inVar_yz[c]=np.zeros(count_yz,dtype=dtype,order='F')
        for c,vi in var_get_yz.items():
    ...
    if xz_slice:
        ...
        var_get_xz={}
        for vi in format_xz_lev:
            if str(rank) in rank_info[f'xz_slice_{vi}']:
                var_name=f"xz_slice_{vi}.{begin_iter}"
                print('var_name ',var_name, vi)
                var_get_xz[var_name]=bpIO.InquireVariable(var_name)
        inVar_xz={}
        for c,vi in var_get_xz.items():
            print('var_get_xz ',c,type(vi))
            vi.SetSelection([start_xz,count_xz])
            inVar_xz[c]=np.zeros(count_xz,dtype=dtype,order='C')
        for c,vi in var_get_xz.items():
            bpFileReader.Get(vi, inVar_xz[c], adios2.Mode.Deferred)
            print('inVar_xz ',c,type(inVar_xz[c]))
        bpFileReader.EndStep()

        print("xz_slice IO done")
        for c,vi in inVar_xz.items():
            print(rank,'write inVar_xz ',c,inVar_xz[c][0,0,0])
        if multifile:
            for c,vi in inVar_xz.items():
                prim2_xz[c[:13]][start_xz[0]:end_xz[0],start_xz[1]:end_xz[1],start_xz[2]:end_xz[2]]=inVar_xz[c]
    ...
    if multifile:
        ...
        prim2.result_prim_0[start[0]:end[0],start[1]:end[1],start[2]:end[2]]=inVar
        prim2.result_prim_1[start[0]:end[0],start[1]:end[1],start[2]:end[2]]=inVar1
        prim2.result_prim_2[start[0]:end[0],start[1]:end[1],start[2]:end[2]]=inVar2
        prim2.result_prim_3[start[0]:end[0],start[1]:end[1],start[2]:end[2]]=inVar3
        prim2.result_prim_4[start[0]:end[0],start[1]:end[1],start[2]:end[2]]=inVar4
        prim2.result_prim_5[start[0]:end[0],start[1]:end[1],start[2]:end[2]]=inVar5
        prim2.result_prim_6[start[0]:end[0],start[1]:end[1],start[2]:end[2]]=inVar6
        prim2.result_prim_7[start[0]:end[0],start[1]:end[1],start[2]:end[2]]=inVar7
        prim2.result_prim_8[start[0]:end[0],start[1]:end[1],start[2]:end[2]]=inVar8
