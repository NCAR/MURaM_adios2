...
//#pragma acc enter data copyin(idx[:3])   
  ...
  double qrho[vsize], hft1[vsize], hfb1[vsize], cm[vsize],
    ...
    hyp_e[vsize],hyp_v[vsize],hyp_B[vsize],CZ_fac[vsize],hyp_par[v_nvar][vsize];
...
#pragma acc parallel loop collapse(2) gang                          \
  ...
  private(offset, var[:v_nvar][:vsize],                             \
    slp[:v_nvar][:vsize],                                           \
    CZ_fac[:vsize], hyp_par[:v_nvar][:vsize],	       		    \
...
#pragma acc loop vector private(node,vv_amb,dn, vv, va2, x2, x4, s, lf, cs2, \
 cfast, CME_mode, rf, hh, cf,dn, vv, bb) 
	...
	*/
	...
	dn = hfb1[i]*tvd_h[0]+(rf*tvd_h[1]+(1.0-rf)*tvd_h[2])*hh+hft1[i]*tvd_h[3];
        hyp_par[0][i] = dn*1.25;
        hyp_par[4][i] = dn;

        dn = hfb1[i]*tvd_h[0]+(rf*tvd_h[1]+(1.0-rf)*tvd_h[2]*tvd_pm_v)*hh+hft1[i]*tvd_h[3]*tvd_pm_v;
        hyp_par[1][i] = dn;
        hyp_par[2][i] = dn;
        hyp_par[3][i] = dn;
        
        dn = hfb1[i]*tvd_h[0]+(rf*tvd_h[1]+(1.0-rf)*tvd_h[2]*tvd_pm_B)*hh+hft1[i]*tvd_h[3]*tvd_pm_B;
        hyp_par[5][i] = dn;
        hyp_par[6][i] = dn;
        hyp_par[7][i] = dn;

        hyp_par[1][i] *= (1.0-CME_mode);
        hyp_par[2][i] *= (1.0-CME_mode);
        hyp_par[3][i] *= (1.0-CME_mode);
        hyp_par[4][i] *= (1.0-CME_mode);
...
#pragma acc loop vector 
	for(i=0;i<sz;i++){
	  hyperdiff[i] = vhyp*sqrt(vsqr[i])*CZ_fac[i];	  
