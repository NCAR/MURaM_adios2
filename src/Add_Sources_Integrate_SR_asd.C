...
void Source_Integrate_Tcheck(const RunData& Run, GridData& Grid, 
  ...
  const int H_loss = max(min(Physics.rt_ext[i_ext_hlines],1),0);
  const int Mg_loss = max(min(Physics.rt_ext[i_ext_mglines],1),0);
  const int Ca_loss = max(min(Physics.rt_ext[i_ext_calines],1),0);
  ...
  double ee_low, ee_up,ee,efaci,e_dmpi, Qtot[vsize], Qthin[vsize];
  ...
  const double rt_step= max(1.0,Physics.rt[i_rt_tstep]);
  const double rt_cfl = Physics.rt[i_rt_cfl]; 
...
#pragma acc loop vector private(node)
      for(i=i_beg;i<=i_end;i++){
        node = off0+i;
        Qtot[i] = Grid.Qtot[node];
	Qthin[i]= Grid.Qthin[node];
      }
    } else if(ext_cor==2){
      #pragma ivdep
#pragma acc loop vector private(node) 
      for(i=i_beg;i<=i_end;i++){
        node = off0+i;
        Qtot[i] = Grid.Qtot[node];
	Qthin[i]= Grid.Qthin[node]+Grid.QH[node]*H_loss
...
#pragma acc loop vector private(node, e_dmpi, efaci, dn) 
    for(i=i_beg;i<=i_end;i++){
      ...
      dn      = rt_cfl*invRdt*Grid.U[node].e;
      efaci = dn/max(dn,fabs(rt_step*Qtot[i]+Qthin[i]));

      // gravity + damping + RT
      dn       = g*Grid.U[node].d-dmp[i];
      e_dmpi = dn*Grid.U[node].M.x;
      Grid.Res[node].M.x += dn;
      Grid.Res[node].e   += e_dmpi + (Qtot[i]+Qthin[i])*efaci;
        // diagnostics
