...
present(Grid[:1], Grid.U[:bufsize], Grid.Qthin[:bufsize],   \
       ...
       Grid.QChr[:bufsize], Grid.QCa[:bufsize],     \
       Grid.QMg[:bufsize], Grid.QH[:bufsize],       \
...
#pragma acc atomic update
    loc[ind+41*ioff] += rfac*(Qthin+QChr+QCa+QMg+QH)*tvar4;
#pragma acc atomic update
    loc[ind+42*ioff] += rfac*tvar1; // div(F_adv)
#pragma acc atomic update
    loc[ind+43*ioff] += rfac*tvar2; // div(F_cond)
#pragma acc atomic update
    loc[ind+44*ioff] += rfac*tvar3; // wlrt
#pragma acc atomic update
    loc[ind+45*ioff] += rfac*tvar5; // m*g + damping
#pragma acc atomic update
    loc[ind+46*ioff] += rfac*tvar6; // Boris
#pragma acc atomic update
    loc[ind+47*ioff] += rfac*tvar7; // Tcheck
#pragma acc atomic update
    loc[ind+48*ioff] += rfac*tvar8; // numerical diff
#pragma acc atomic update
    loc[ind+49*ioff] += rfac*Qres;
#pragma acc atomic update
    loc[ind+50*ioff] += rfac*Qvis;
  }
}
}
