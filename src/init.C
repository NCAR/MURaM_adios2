...
int Initialize(RunData& Run,GridData& Grid,
  ...
  if( rank==0 ) {
    ...
    getvar(&Run.restartfreq,"restartfreq","int",datafile);
    ...
    getvar(&Run.enable_adios2,"enable_adios2","int",datafile);
    getvar(&Run.enable_adios2_double,"enable_adios2_double","int",datafile);
